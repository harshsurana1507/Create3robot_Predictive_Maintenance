...
class StopStatusSubscriber(Node):
    def __init__(self):
        super().__init__('stop_status_subscriber')

        topic_name = '/robot01/stop_status'
        ...
        self.worksheet.title = 'stop Status'
        self.worksheet.append(["Timestamp", "Is robot stopped"])
    ...
    def process_buffered_messages(self):
        ...
        self.get_logger().info(f"Stop status: \n - is robot stopped: {latest_msg.is_stopped} \n \n")
        #current time and date
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        if latest_msg.is_stopped == True:
            a = "1"
        
        if latest_msg.is_stopped == False:
            a="0"
    ...
    def save_to_excel(self):
        ...
        # Save the workbook
        self.workbook.save("stop_status.xlsx")
        self.get_logger().info('Data saved to stop_status.xlsx')
...
def main(args=None):
    ...
    stop_status_subscriber = StopStatusSubscriber()
    try:
        rclpy.spin(stop_status_subscriber)
    ...
    # when the garbage collector destroys the node object)
        ...
        if stop_status_subscriber:     # this if checks whether stop_status_subscriber exist or not
            stop_status_subscriber.destroy_node()  #function to destroy the node is called 
