...
class KidnapStatusSubscriber(Node):
    ...
    def process_buffered_messages(self):
        ...
        self.get_logger().info(f"Kidnap status: \n - is robot kidnapped: {latest_msg.is_kidnapped} \n \n")
        #current time and date
        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        if latest_msg.is_kidnapped == True:
            a = "1"
        
        if latest_msg.is_kidnapped == False:
            a="0"
    ...
    def save_to_excel(self):
        ...
        self.get_logger().info('Data saved to kidnap status.xlsx')
...
def main(args=None):
    ...
    # when the garbage collector destroys the node object)
        ...
        if kidnap_status_subscriber:   # this if checks whether kidnap_status_subscriber exist or not
            kidnap_status_subscriber.destroy_node() #function to destroy the node is called 
